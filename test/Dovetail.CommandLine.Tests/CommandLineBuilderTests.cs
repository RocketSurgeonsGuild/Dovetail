...
namespace Dovetail.CommandLine.Tests;
...
public class CommandLineBuilderTests() : AutoFakeTest<TestRecord>(TestRecord.Create())
{
    ...
    {
        var builder = DovetailContextBuilder.Create(_ => []);

        var a = () => builder.PrependJoint(A.Fake<ICommandLineDovetail>());
        a.ShouldNotThrow();
        a = () => builder.AppendJoint(A.Fake<ICommandLineDovetail>());
        a.ShouldNotThrow();
        a = () => builder.PrependDelegate(new ServiceJoint((context, services) => { }), null, null);
        a.ShouldNotThrow();
        a = () => builder.AppendDelegate(new ServiceJoint((context, services) => { }), null, null);
        a.ShouldNotThrow();
    }
    ...
    public async Task ShouldNotBeEnabledIfNoCommandsAreConfigured(CancellationToken cancellationToken)
    {
        using var host = await Host
                              ...
                              .ConfigureDovetail(_ => { }, cancellationToken: cancellationToken);
...
, cancellationToken: cancellationToken);
    ...
    public async Task Sets_Values_In_Commands(CancellationToken cancellationToken)
    {
        using var host = await Host
                              ...
                              .ConfigureDovetail(b => b.ConfigureCommandLine((context, builder) => builder.AddCommand<CommandWithValues>("cwv")), cancellationToken: cancellationToken);
