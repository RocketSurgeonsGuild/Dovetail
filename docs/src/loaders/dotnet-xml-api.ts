...
function typeKeyFromRef(ref: string): string {
    ...
    const typeJoints = kind === 'T' ? raw : splitDeclaringType(raw).declaringType;
    return stripArity(typeJoints).replace(/\+/g, '.');
}
...
export function dotnetXmlApiLoader(options: DotnetXmlApiLoaderOptions): Loader {
    ...
    return {
        ...
        async load(context: LoaderContext): Promise<void> {
            ...
            //    but only if there's no hand-written `api/index.md` claiming that same id, since a
            //    curated landing page (see src/content/docs/api/index.md) is preferred over the
            //    auto-generated flat listing.
            if (!context.store.has(opts.basePath)) {
